...
function setupRouting() {
  ...
  if (isLecturerPage) {
    ...
    elements.lecturerAuthForm.onsubmit = (event) => {
      event.preventDefault();
      
      // Mengambil password dari CONFIG.password ("test123")
      if (elements.lecturerPasswordInput.value === CONFIG.password) {
        elements.lecturerAuthModal.classList.remove("active");
        elements.lecturerPage.hidden = false;
        setStatus("Akses dosen diberikan.");
      } else {
        elements.lecturerAuthError.textContent = "Password salah! Akses ditolak.";
        elements.lecturerPasswordInput.value = "";
        elements.lecturerPasswordInput.focus();
      }
    };
...
document.addEventListener("visibilitychange", () => {
  if (state.isRunning && !state.isLocked && !state.examEnded) {
    if (document.hidden) {
      applyPenalty(
        "switch-tab",
        "Pelanggaran! Anda terdeteksi meninggalkan halaman ujian (pindah tab/minimize)."
      );
    }
  }
});
...
window.addEventListener("blur", () => {
  if (state.isRunning && !state.isLocked && !state.examEnded) {
    applyPenalty(
      "switch-window",
      "Pelanggaran! Anda terdeteksi membuka aplikasi lain di luar browser."
    );
  }
});
